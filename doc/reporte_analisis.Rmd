...
Dado que los datos que obtuvimos de Twitter ya limpiados y estandarizados es un conjunto de datos multidimensional,se decidió utilizar el Análisisde Componentes Principales para poder reducir las dimensiones del problema. Se plantea calcular las componentes principales, y escoger el número de estas que determinen más del 95% de la varianza, junto con el "Codo de Jambú" para demostrar que efectivamente se escogió el número óptimo de componentes principales. Con los componentes principales escogidos, y simplificando los datos a un menor espacio de dimensiones, se procederá a buscar patrones en los datos usando la técnica de k-means para separar los datos en clústeres (grupos), en nuestro caso particular el patrón es la frecuencia con la cual se repite, y luego poder agregarlos a un data frame a cada registro correspondiente. 
```{r eval=TRUE, message= FALSE}
...
Al graficar el plano principal obtenemos que a  simple vista no se pueden distinguir los grupos, sin embargo basándonos en el resultado del círculo de correlación la cantidad de grupos posibles podría ser 2 o 3. Tambien tenemos valores atípicos como lo es "cambio".
...
####K-medias
Se procedió a gráficar el k-means de los componentes principales con colores diferentes para cada grupo, y su centroide. Luego se creó un objeto con los valores de los grupos asignados a cada registro
En K-Medias podemos analizar todo el dataset, ya que se puede graficar la nube de puntos de tal forma que se puede analizar con facilidad.
